...
`for` is Go’s primary looping construct.
...
Variables declared in `if` statements are available in the current and subsequent branches.
...
An array type specifies a length and an element type:
...
// Compiler can count array elements
...
## Growing Slices (Using copy and append Functions)
